...
namespace NetworkScanner.Upnp
{
    ...
    public sealed class UpnpSmartControlPoint
    {
        ...
        //private string searchFilter = "ssdp:all";
        //"ssdp:all"; //

        public UpnpSmartControlPoint()
        {
            deviceFactory = new UPnPDeviceFactory();
            ...
            //this.genericControlPoint.syn += OnReceiveSing;
            this.genericControlPoint.FindDeviceAsync(searchFilter);
        }

        //private void OnReceiveSing(IAsyncResult obj){throw new NotImplementedException();}
        ...
        internal void SSDPNotifySink(IPEndPoint source,
                                     IPEndPoint local,
                                     Uri LocationURL,
                                     bool IsAlive,
                                     string USN,
            ...
            if (SearchTarget == searchFilter)
            {
                ...
                else
                {
                    lock (this.deviceTableLock)
                    {
                        ...
                        var enumerator = deviceTable.GetEnumerator();

                        while (enumerator.MoveNext())
                        {
                            var item = ((DictionaryEntry)enumerator.Current);
                            var info = (DeviceInfo)item.Value;

                            var fd = new FoundDevice
                            {
                                DeviceId = info.UDN,
                                IpAddress = info.SourceEP.Address.ToString(),
                                FoundUsing = "Upnp",
                                FoundAt = DateTime.Now
                            };

                            var queryResult = LoadDeviceDetails(info.BaseURL);
